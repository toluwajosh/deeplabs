...
def get_palette(dataset):
    ...
    if dataset == "pascal" or dataset == "pascal_toy":
        ...
        res = np.array(
            [[res[i], res[i + 1], res[i + 2]] for i in range(0, len(res), 3)],
            dtype=np.uint8,
        )
        return res
...
class Visualize(object):
    ...
    def restore(self, image, mean=None, std=None):
        ...
        elif self.dataset == "pascal" or self.dataset == "pascal_toy":
    ...
    def predict_color(self, preds, origins, names, save_dir="./"):
        for i in range(len(preds)):
            ...
            if pred.shape != origin.shape:
                ...
                if self.dataset == "pascal" or self.dataset == "pascal_toy":
    ...
    def predict_examine(self, preds, targets, origins, names, save_dir="./"):
        for i in range(len(preds)):
            ...
            target = targets[i]
            origin = origins[i]
            saveas = os.path.join(save_dir, names[i])
            origin = self.restore(origin)
