...
class DeepLabv3(nn.Module):
    def __init__(
        ...
        self.backbone = build_backbone(
            backbone, output_stride, Norm, dec=False, abn=abn
        )
...
class LaneDeepLab(DeepLab):
    def __init__(self, args, num_classes=21, freeze_bn=False, abn=False, deep_dec=True):
        ...
        self.backbone = build_backbone(args)
        self.aspp = build_aspp(args.backbone, args.out_stride, norm)
        # We add one more channel for outputing lane lines
        if self.deep_dec:
            self.decoder = build_decoder(num_classes + 1, args.backbone, norm)
